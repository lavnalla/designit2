...
export function Studio({ onBack }: { onBack: () => void }) {
  ...
  const syncWorkspaceToTryOn = (): Promise<string | null> => {
    return new Promise((resolve) => {
      ...
      try {
        ...
        const viewBox =
          svgEl.getAttribute("viewBox") || `0 0 ${width} ${height}`;
    ...
    try {
      ...
      if (data.resultImage) {
        setWorkspaceShapes((prev) =>
          prev.map((s) =>
            s.id === selectedShapeId ? { ...s, img: data.resultImage } : s,
          ),
        );
      } else if (data.error) {
  ...
  useEffect(() => {
    ...
    const generateFlatCanvasSnapshot = () => {
      try {
        const activeShapeObj = workspaceShapes?.find(
        ...
        baseImg.onload = () => {
          ...
          const isBlouse =
            activeShapeObj.id?.includes("blouse") ||
            (activeShapeObj.img && activeShapeObj.img.includes("blouse"));
          if (isBlouse) {
  ...
  const addShapeToCanvas = useCallback(
    (
      ...
      const newStroke: Stroke = {
        ...
        baseFill:
          type !== "line" && type !== "curve" && !isGem ? "#ffffff" : undefined,
  ...
  const pasteFromSelection = useCallback(() => {
    ...
    const newShapes = clipboard.shapes.map((shape) => ({
      ...
      position: {
        x: (shape.position?.x ?? 0) + PASTE_OFFSET,
        y: (shape.position?.y ?? 0) + PASTE_OFFSET,
      },
        ...
        if (
          ...
          return {
            ...
            dots: s.dots.filter(
              (d) =>
                Math.hypot(
                  s.position.x + d.x * s.scale - x,
                  s.position.y + d.y * s.scale - y,
                ) > ERASE_RADIUS,
            ),
          };
        }
  ...
  return (
    <div
      className="flex min-h-[100dvh] w-full flex-col overflow-hidden bg-gradient-to-br from-slate-50 via-white to-stone-50 text-slate-900 select-none"
    ...
    >
      {aiDraping.active && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/60 backdrop-blur-md">
          <div className="bg-white p-6 rounded-2xl shadow-2xl flex flex-col items-center gap-4">
            ...
            <p className="text-sm font-black uppercase text-purple-600 animate-pulse">
              {aiDraping.text}
            </p>
          </div>
        </div>
          ...
          >
            ...
            <div className="mt-8 flex gap-3">
              ...
              <button
                ...
                className="flex-1 bg-gradient-to-r from-purple-500 to-pink-500 text-white py-3 rounded-xl text-[10px] font-black uppercase hover:shadow-lg transition-all"
              >
                Add to Canvas
              </button>
            </div>
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-4 max-h-[60vh] overflow-y-auto pr-2">
              ...
              >
                ...
                <span className="text-[10px] font-black uppercase text-slate-600 group-hover:text-indigo-600">
                  Square
                </span>
              </button>
              <button
              ...
              >
                <svg
                  className="w-12 h-12 text-slate-200 group-hover:text-indigo-400 transition-colors mb-2"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
      ...
      <header className="h-16 flex items-center justify-between px-2 lg:px-8 bg-gradient-to-r from-yellow-600 via-yellow-500 to-yellow-600 border-b-4 border-[#B87333] shrink-0 z-[100] shadow-md">
        <div className="flex items-center gap-2 sm:gap-4">
          <button
            ...
            className="lg:hidden bg-gradient-to-r from-yellow-500 to-yellow-600 text-white px-3 py-2 rounded-xl text-[9px] font-black uppercase shadow-md hover:shadow-lg transition-all"
          >
            Trace
          </button>
          <div
            onClick={onBack}
            className="flex flex-col cursor-pointer active:scale-95 px-2"
          >
            <span className="text-[10px] sm:text-[11px] font-black uppercase tracking-[0.2em] text-white drop-shadow-sm">
              DesignIt{" "}
              <span className="text-[#e0f2fe] drop-shadow-[0_0_2px_rgba(255,255,255,0.8)]">
                ♦
              </span>
            </span>
            <span className="hidden xs:block text-[7px] font-medium uppercase text-yellow-100">
              Studio
            </span>
          </div>
          <div className="hidden md:flex items-center gap-4">
            <Link
              href="/"
              className="text-white hover:text-yellow-100 font-medium text-[9px] uppercase transition-colors drop-shadow-sm"
            >
              Home
            </Link>
            <Link
        ...
        <div className="absolute left-1/2 -translate-x-1/2 hidden lg:flex items-center gap-2 px-4 py-2 bg-gradient-to-br from-slate-100 to-slate-200 rounded-full border-2 border-[#B87333] shadow-lg animate-pulse">
          ...
          <span className="w-5 h-5 bg-gradient-to-br from-rose-500 to-rose-600 text-white rounded-full flex items-center justify-center text-[10px] font-black ring-1 ring-white">
            ?
          </span>
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          <button
            ...
            className="px-2 sm:px-4 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-full text-[8px] sm:text-[9px] font-black uppercase shadow-md hover:shadow-lg transition-all flex items-center gap-1"
          >
          ...
          <button
            id="undo-btn"
            onClick={undo}
            className="px-2 sm:px-4 py-2 bg-pink-50 text-pink-600 rounded-full text-[8px] sm:text-[9px] font-black uppercase border border-pink-100 hover:bg-pink-100"
          >
            Undo
          </button>
          <button
            ...
            className="px-2 sm:px-4 py-2 bg-emerald-50 text-emerald-600 rounded-full text-[8px] sm:text-[9px] font-black uppercase border border-emerald-100 hover:bg-emerald-100"
          >
            Reset
          </button>
          <button
            ...
            className="px-2 sm:px-4 py-2 bg-blue-50 text-blue-600 rounded-full text-[8px] sm:text-[9px] font-black uppercase border border-blue-100 hover:bg-blue-100"
          >
            Download
          </button>
          <button
            ...
            className={`px-2 sm:px-4 py-2 rounded-full text-[8px] sm:text-[9px] font-black uppercase border transition-all ${globalShowDots ? "bg-yellow-50 text-yellow-700" : "bg-white text-slate-400"}`}
          >
            Dots
          </button>
          <button
            ...
            className={`px-2 sm:px-4 py-2 rounded-full text-[8px] sm:text-[9px] font-black uppercase border transition-all ${isLocked ? "bg-sky-500 text-white" : "bg-white text-sky-500"}`}
          >
            Lock
          </button>
        </div>
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden relative">
        <aside
          className={`fixed lg:static inset-0 lg:w-[320px] bg-slate-50 lg:border-r-2 border-slate-200 flex flex-col z-[200] lg:z-0 transition-transform shadow-lg ${isSidebarOpen ? "translate-y-0" : "translate-y-full lg:translate-y-0"}`}
        >
          <div className="p-6 shrink-0 bg-white border-b-2 border-slate-200">
            <div className="flex justify-between items-center mb-6">
              ...
              <button
                ...
                className="lg:hidden text-slate-500 hover:text-slate-800 text-xs font-bold transition-colors"
              >
                CLOSE ✕
              </button>
            </div>
            <div className="grid grid-cols-2 gap-2 mb-3">
              ...
              <button
                ...
                className="bg-gradient-to-br from-slate-500 to-slate-600 text-white py-1.5 px-2 rounded-md text-[8px] font-bold uppercase shadow-sm hover:shadow-md transition-all"
              >
                Upload
                <input
                  type="file"
              ...
              <button
                ...
                className="bg-gradient-to-br from-slate-800 to-slate-900 text-yellow-300 py-1.5 px-2 rounded-md text-[8px] font-bold uppercase shadow-sm hover:shadow-md transition-all disabled:opacity-30"
              >
                Add to Canvas
              </button>
            </div>
              ...
              >
                ...
                {candidates.map((c, idx) => (
                  <path
                    ...
                    d={c.d}
                    fill={
                      c.selected ? "rgba(251, 146, 60, 0.5)" : "transparent"
                    }
                    stroke={c.selected ? "#f97316" : "#cbd5e1"}
                    strokeWidth={4}
        ...
        <main className="flex-1 min-w-0 bg-[#F9F9FB] relative overflow-hidden">
          {/* Top toolbar removed - Download moved into header controls */}
          {ghostCursor.active && (
            <div
              className="fixed pointer-events-none z-[1000] transition-all duration-700 ease-in-out flex flex-col items-center"
              style={{
                left: ghostCursor.x,
                top: ghostCursor.y,
            ...
            >
              {contextMenu.type === "selection" ? (
                <>
                  ...
                  <button
                    onClick={ungroupItemsFromSelection}
                  ...
                  <button
                    onClick={() => extractSelection(false)}
                  ...
                  {clipboard &&
                    (clipboard.shapes.length > 0 ||
                      ...
                      <button
                        ...
                        className="w-full text-left px-4 py-2 hover:bg-green-50 text-[9px] font-black uppercase text-green-600"
                      >
                        📌 Paste
                      </button>
                    )}
                  ...
                  <button
                    onClick={() => {
                      if (
                        ...
                        bringToFront(contextMenu.id, contextMenu.type);
                    }}
                  ...
                  <button
                    onClick={() => {
                      if (
                        ...
                        sendToBack(contextMenu.id, contextMenu.type);
                    }}
                    className="w-full text-left px-4 py-2 text-[9px] font-black uppercase border-b border-slate-100 hover:bg-slate-50"
                  >
                    Send to Back
                  ...
                  {contextMenu.type === "shape" &&
                    !workspaceShapes.find((s) => s.id === contextMenu.id)
                      ...
                      <button
                        ...
                        className="w-full text-left px-4 py-2 hover:bg-purple-50 text-[9px] font-black uppercase border-b border-slate-100 text-purple-600"
                      >
                        🎀 Drape to Mannequin
                      </button>
                    )}
          ...
          <div className="absolute left-2 top-1/2 -translate-y-1/2 flex flex-col gap-2 p-2 bg-white/80 rounded-[2rem] shadow-xl z-50">
            {(
              ...
              <button
                ...
                className={`w-10 h-10 flex items-center justify-center rounded-xl transition-all ${activeTool === t ? "bg-yellow-400 text-black" : "text-slate-400 hover:bg-slate-100"}`}
              >
                <span className="text-[10px] font-black uppercase">
                  {t === "ghost" ? "👻" : t === "scissor" ? "✂️" : t.charAt(0)}
                </span>
              </button>
            ))}
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      >
                        ...
                        />
                        <div
                          className={`${keepOriginalColor ? "bg-gradient-to-r from-amber-500 to-orange-500" : "bg-slate-200"} w-12 h-6 rounded-full p-1 transition-colors`}
                        >
                          <div
                    ...
                    <div>
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-2">
                        <button
                          ...
                          className="px-3 py-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl text-[10px] font-black uppercase shadow-md hover:shadow-lg"
                        >
                          Pick & Remove
                        </button>
                        <button
          ...
          <div className="shrink-0 p-4 pb-2 flex gap-2 items-center bg-white border-b border-slate-200 shadow-sm z-10 sticky top-0 flex-wrap">
            ...
            <button
              onClick={() => setWorkspaceBgColor("white")}
              className={`px-2 py-1 rounded text-[8px] font-bold transition-all ${workspaceBgColor === "white" ? "ring-2 ring-blue-500 bg-white text-slate-900" : "bg-white border border-slate-300 text-slate-600 hover:border-slate-400"}`}
            >
              White
            </button>
            <button
              onClick={() => setWorkspaceBgColor("amber")}
              className={`px-2 py-1 rounded text-[8px] font-bold transition-all ${workspaceBgColor === "amber" ? "ring-2 ring-blue-500 bg-amber-200 text-slate-900" : "bg-amber-100 border border-amber-300 text-slate-600 hover:border-amber-400"}`}
            >
              Amber
            </button>
            <button
              onClick={() => setWorkspaceBgColor("slate")}
              className={`px-2 py-1 rounded text-[8px] font-bold transition-all ${workspaceBgColor === "slate" ? "ring-2 ring-blue-500 bg-slate-300 text-white" : "bg-slate-200 border border-slate-400 text-slate-600 hover:border-slate-500"}`}
            >
              Slate
            </button>
            <button
              onClick={() => setWorkspaceBgColor("gray")}
              className={`px-2 py-1 rounded text-[8px] font-bold transition-all ${workspaceBgColor === "gray" ? "ring-2 ring-blue-500 bg-gray-200 text-slate-900" : "bg-gray-100 border border-gray-300 text-slate-600 hover:border-gray-400"}`}
            >
              Gray
            </button>
            <button
              onClick={() => setWorkspaceBgColor("blue")}
              className={`px-2 py-1 rounded text-[8px] font-bold transition-all ${workspaceBgColor === "blue" ? "ring-2 ring-blue-500 bg-blue-200 text-slate-900" : "bg-blue-100 border border-blue-300 text-slate-600 hover:border-blue-400"}`}
            >
              Blue
            </button>
          ...
          <div
            ref={canvasRef}
            className="h-full min-h-0 w-full overflow-auto overscroll-contain px-2 py-2 sm:px-4 sm:py-4 lg:px-6 lg:py-6"
            onPointerDown={(e) => {
            ...
            onPointerUp={(e) => {
              ...
              if (
                ...
                if (
                  (e.nativeEvent as PointerEvent).button === 2 ||
                  e.pointerType === "touch"
                ) {
                  setContextMenu({
                    x: e.clientX,
          ...
          >
            ...
            <div className="min-w-[920px] sm:min-w-[1200px] lg:min-w-[1400px] min-h-[820px] sm:min-h-[960px] lg:min-h-[1100px]">
              <svg
                id="workspace-svg"
                ref={workspaceRef}
                viewBox="0 0 1200 900"
                className={`block h-[900px] w-[1200px] rounded-2xl border shadow-2xl sm:h-[960px] sm:w-[1400px] lg:h-[1100px] lg:w-[1600px] sm:rounded-[2rem] lg:rounded-[3rem] ${getWorkspaceBgClasses()}`}
                    ...
                    if (item.type === "shape") {
                      ...
                      return (
                        ...
                        >
                          {s.baseFill && (
                            <path
                              d={generatePathData(s.points, s.closed ?? false)}
                              fill={s.baseFill}
                              pointerEvents="none"
                              strokeLinecap="round"
          ...
          <button
            ...
            className="fixed bottom-6 right-6 w-12 h-12 bg-gradient-to-br from-amber-500 to-orange-500 text-white rounded-full font-black shadow-2xl border-4 border-white hover:scale-110 transition-transform disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
          >
            ?
          </button>
        </main>
      ...
      <div className="w-full flex flex-col md:flex-row items-center gap-2 px-4 py-3 bg-white border-t border-slate-200 shadow-inner z-50 sticky bottom-0">
        {refineError && (
          <div className="w-full text-red-600 text-xs mb-1 text-center">
            {refineError}
          </div>
        )}
        <input
        ...
        <button
          onClick={async () => {
            if (!showTryOn) {
              console.log(
                "🎒 [STUDIO] Preparing workspace asset before initializing camera stream...",
              );
              ...
              if (!readyAsset) {
                alert(
                  "Could not process studio workspace asset safely. Try again.",
                );
                return;
              }
